...
void vkglTF::Texture::destroy()
{
	if (core)
	{
		...
		vkDestroyImage(core->GetDevice(), image, nullptr);
		vkFreeMemory(core->GetDevice(), deviceMemory, nullptr);
		vkDestroySampler(core->GetDevice(), sampler, nullptr);
	}
}

void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, VulkanCore* core, VkQueue copyQueue)
{
	this->core = core;
	...
	if (!isKtx) {
		...
		vkGetPhysicalDeviceFormatProperties(core->GetPhysicalDevice(), format, &formatProperties);
		...
		VK_CHECK(vkCreateBuffer(core->GetDevice(), &bufferCreateInfo, nullptr, &stagingBuffer));
		vkGetBufferMemoryRequirements(core->GetDevice(), stagingBuffer, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = core->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		VK_CHECK(vkAllocateMemory(core->GetDevice(), &memAllocInfo, nullptr, &stagingMemory));
		VK_CHECK(vkBindBufferMemory(core->GetDevice(), stagingBuffer, stagingMemory, 0));

		uint8_t* data;
		VK_CHECK(vkMapMemory(core->GetDevice(), stagingMemory, 0, memReqs.size, 0, (void**)&data));
		memcpy(data, buffer, bufferSize);
		vkUnmapMemory(core->GetDevice(), stagingMemory);
		...
		VK_CHECK(vkCreateImage(core->GetDevice(), &imageCreateInfo, nullptr, &image));
		vkGetImageMemoryRequirements(core->GetDevice(), image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = core->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK(vkAllocateMemory(core->GetDevice(), &memAllocInfo, nullptr, &deviceMemory));
		VK_CHECK(vkBindImageMemory(core->GetDevice(), image, deviceMemory, 0));
		...
		core->flushCopyCommandBuffer(copyCmd, copyQueue, true);

		vkFreeMemory(core->GetDevice(), stagingMemory, nullptr);
		vkDestroyBuffer(core->GetDevice(), stagingBuffer, nullptr);

		// Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
		VkCommandBuffer blitCmd = core->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		core->flushCopyCommandBuffer(blitCmd, copyQueue, true);
	}
	else {
...

		{
			...
			if(fileStream.fail())
			{
				VK_ERROR("Could not load texture from {0}\n\nMake sure the assets submodule has been checked out and is up-to-date.", filename);
			}
		}
		...
		VkBufferCreateInfo bufferCreateInfo = init::createBufferInfo(ktxTextureSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
		bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		VK_CHECK(vkCreateBuffer(core->GetDevice(), &bufferCreateInfo, nullptr, &stagingBuffer));
		...
		vkTool::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
		...
		vkTool::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
	...
	VK_CHECK(vkCreateSampler(core->GetDevice(), &samplerInfo, nullptr, &sampler));
	...
	VK_CHECK(vkCreateImageView(core->GetDevice(), &viewInfo, nullptr, &view));
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	VK_CHECK(vkAllocateDescriptorSets(core->GetDevice(), &descriptorSetAllocInfo, &descriptorSet));
	...
	vkUpdateDescriptorSets(core->GetDevice(), static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
...
/*
	glTF mesh
*/
vkglTF::Mesh::Mesh(VulkanCore* core, glm::mat4 matrix) {
	this->core = core;
	this->uniformBlock.matrix = matrix;
	VK_CHECK(core->createBuffer(
		...
		&uniformBlock));
	VK_CHECK(vkMapMemory(core->GetDevice(), uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
	uniformBuffer.descriptor = { uniformBuffer.buffer, 0, sizeof(uniformBlock) };
};

vkglTF::Mesh::~Mesh() {
	vkDestroyBuffer(core->GetDevice(), uniformBuffer.buffer, nullptr);
	vkFreeMemory(core->GetDevice(), uniformBuffer.memory, nullptr);
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	VkBuffer stagingBuffer;
	VkDeviceMemory stagingMemory;
	VkBufferCreateInfo bufferCreateInfo = init::createBufferInfo(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
	bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
	VK_CHECK(vkCreateBuffer(core->GetDevice(), &bufferCreateInfo, nullptr, &stagingBuffer));
	...
	imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	VK_CHECK(vkCreateImage(core->GetDevice(), &imageCreateInfo, nullptr, &emptyTexture.image));

	vkGetImageMemoryRequirements(core->GetDevice(), emptyTexture.image, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = core->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	VK_CHECK(vkAllocateMemory(core->GetDevice(), &memAllocInfo, nullptr, &emptyTexture.deviceMemory));
	VK_CHECK(vkBindImageMemory(core->GetDevice(), emptyTexture.image, emptyTexture.deviceMemory, 0));
	...
	VkCommandBuffer copyCmd = core->createCopyCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
	vkTool::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
	vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
	vkTool::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
	core->flushCopyCommandBuffer(copyCmd, transferQueue);
	...
	VK_CHECK(vkCreateSampler(core->GetDevice(), &samplerCreateInfo, nullptr, &emptyTexture.sampler));
	...
	VK_CHECK(vkCreateImageView(core->GetDevice(), &viewCreateInfo, nullptr, &emptyTexture.view));
...
vkglTF::Model::~Model()
{
	vkDestroyBuffer(core->GetDevice(), vertices.buffer, nullptr);
	vkFreeMemory(core->GetDevice(), vertices.memory, nullptr);
	vkDestroyBuffer(core->GetDevice(), indices.buffer, nullptr);
	vkFreeMemory(core->GetDevice(), indices.memory, nullptr);
	...
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(core->GetDevice(), descriptorSetLayoutUbo, nullptr);
		descriptorSetLayoutUbo = VK_NULL_HANDLE;
	}
	if (descriptorSetLayoutImage != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(core->GetDevice(), descriptorSetLayoutImage, nullptr);
...
void vkglTF::Model::loadNode(vkglTF::Node *parent, const tinygltf::Node &node, uint32_t nodeIndex, const tinygltf::Model &model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
{
	...
	if (node.mesh > -1) {
		...
		Mesh *newMesh = new Mesh(core, newNode->matrix);
...
void vkglTF::Model::loadImages(tinygltf::Model &gltfModel, VulkanCore* core, VkQueue transferQueue)
{
	for (tinygltf::Image &image : gltfModel.images) {
		vkglTF::Texture texture;
		texture.fromglTfImage(image, path, core, transferQueue);
...
void vkglTF::Model::loadMaterials(tinygltf::Model &gltfModel)
{
	...
	materials.push_back(Material(core));
}
...
void vkglTF::Model::loadFromFile(std::string filename, VulkanCore* core, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if (fileLoaded) {
		if (!(fileLoadingFlags & FileLoadingFlags::DontLoadImages)) {
			loadImages(gltfModel, core, transferQueue);
		}
		...
		// TODO: throw
		VK_ERROR("Could not load glTF file \"{0}\":  {1}", filename, error);
	...
	core->flushCopyCommandBuffer(copyCmd, transferQueue, true);

	vkDestroyBuffer(core->GetDevice(), vertexStaging.buffer, nullptr);
	vkFreeMemory(core->GetDevice(), vertexStaging.memory, nullptr);
	vkDestroyBuffer(core->GetDevice(), indexStaging.buffer, nullptr);
	vkFreeMemory(core->GetDevice(), indexStaging.memory, nullptr);
	...
	VK_CHECK(vkCreateDescriptorPool(core->GetDevice(), &descriptorPoolCI, nullptr, &descriptorPool));
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			...
			VK_CHECK(vkCreateDescriptorSetLayout(core->GetDevice(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
		}
		for (auto node : nodes) {
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			...
			descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
			descriptorLayoutCI.pBindings = setLayoutBindings.data();
			VK_CHECK(vkCreateDescriptorSetLayout(core->GetDevice(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
		}
		for (auto& material : materials) {
			if (material.baseColorTexture != nullptr) {
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
		descriptorSetAllocInfo.descriptorSetCount = 1;
		VK_CHECK(vkAllocateDescriptorSets(core->GetDevice(), &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
